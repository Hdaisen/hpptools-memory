...
export function resolveLink(link, cwd) {
 ...
 * @property {string} file        相对路径（相对 memory dir，含 .md）
 * @property {string} section     完整的 ## 标题
...
export function parseMemoryEntries(content, relativePath) {
  const entries = [];
...
export function extractRelatedLinks(text) {
  const links = [];
 ...
 */
export function readMemoryIndex(cwd, maxChars = 2500) {
  const scopes = [
 ...
 */
export function searchMemories(userPrompt, cwd, maxResults = 5) {
 ...
 * @property {string} filePath    指向 SKILL.md 的绝对路径
 * @property {"project"|"global"} scope  存储范围
 * @property {string[]} [aliases] 中文/别名关键词（frontmatter aliases 字段）
 */

/**
 * Parse SKILL.md frontmatter: extract name, description, and optional aliases.
 * Format mirrors Pi's agent skills specification.
 */
function parseSkillFrontmatter(content) {
  // CRLF-tolerant: GitHub skill repos often ship CRLF line endings.
 ...
 * Only the "Trigger keywords:" section counts — those are the author-curated
 * high-discrimination terms. Falling back to the whole description would
 * surface generic library-wide words (e.g. "godot") and over-inject.
 */
function extractTriggerKeywords(description) {
  const m = description.match(/trigger\s*keywords?\s*:\s*([^\n.]+)/i);
  if (!m) return [];
  return m[1]
    .split(/[,，]/)
...
/**
 * Filter out low-discrimination keywords — terms that appear in too many
 * skills (e.g. "godot" appears in every GD skill description). Injecting all
 * matches for such a term = Context Storm. A keyword is "generic" when its
 * document frequency ≥ max(3, 60% of skills); such terms carry no signal.
 */
function filterGenericKeywords(keywords, skills) {
  if (skills.length < 4) return keywords; // too few skills to judge frequency
  const docFreq = new Map();
  for (const s of skills) {
    const haystack = [s.name, s.description, ...(s.aliases ?? [])]
      .join(" ")
      .toLowerCase();
    for (const kw of keywords) {
      if (haystack.includes(kw)) docFreq.set(kw, (docFreq.get(kw) ?? 0) + 1);
    }
  }
  const threshold = Math.max(3, Math.floor(skills.length * 0.6));
  return keywords.filter((kw) => (docFreq.get(kw) ?? 0) < threshold);
}

/**
 * Match skills against user prompt using bidirectional keyword overlap.
 * Forward: prompt keywords appear in skill name/description/aliases (Chinese
 * aliases make CJK prompts hit English-described skills).
 * Backward: skill's trigger keywords (GD style "Trigger keywords:" section or
 * extracted technical terms) appear in the prompt — catches users typing raw
 * English terms like "tilemap" or "combat" even when their prompt is Chinese.
 * Generic low-discrimination keywords (e.g. "godot") are filtered first to
 * avoid injecting every skill in a themed library (Context Storm).
 * Returns skills whose description contains prompt keywords.
 */
export function matchSkills(userPrompt, skills) {
  if (!userPrompt || skills.length === 0) return [];
  const keywords = extractKeywords(userPrompt);
  if (keywords.length === 0) return [];
  const specific = filterGenericKeywords(keywords, skills);
  if (specific.length === 0) return []; // only generic terms → nothing to match on
  const promptLower = userPrompt.toLowerCase();

  return skills.filter((skill) => {
    // Forward: prompt keywords ∈ name/description/aliases
    const haystack = [skill.name, skill.description, ...(skill.aliases ?? [])]
      .join(" ")
      .toLowerCase();
    if (specific.some((kw) => haystack.includes(kw))) return true;

    // Backward: skill trigger keywords ∈ prompt
    const triggerKeywords = extractTriggerKeywords(skill.description);
    return triggerKeywords.some((tk) => promptLower.includes(tk));
  });
}
...
export function readLinkedContent(links, cwd, keywords = []) {
  const results = [];
